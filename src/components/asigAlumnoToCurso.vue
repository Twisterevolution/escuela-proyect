	...
	<v-row>
		<v-col cols="12" class="text-start title-color-bg ">
			<h2 class="title-color-bg white--text">
				<v-btn class="btnseparate" color="success" to="./matriculas">VOLVER</v-btn>
...
export default {
	...
	data() {
		return {
			...
			cabecera: [
				...
				{ text: "Curso asignado", value: "alumno.curso.nombreCurso", align: "center" },
	...
	methods: {
        ...
        listaCursos:  function(){
            ...
            .then(res=>{
              ...
              const cursosFiltrados = cursos.filter(x=>{
                  return x.anioAcademico.id == this.$store.state.anioAcademicoData.id
              })
        ...
        alumnosAnioAcademico: function(){
              let datosBusqueda = {
               anioAcademico_id : this.$store.state.anioAcademicoData.id,
               nivel_id : this.id_nivelSeleccionado  
            }
            console.log(datosBusqueda);
             this.cargandodatos = true
            axios.post('/api/matriculas/searchAnioAcademicoNivel', datosBusqueda)
            .then(res=>{
